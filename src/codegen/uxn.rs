...
pub unsafe fn generate_program(
    ...
    generate_asm_funcs(output, da_slice((*p).asm_funcs), &mut assembler);
    generate_extrns(da_slice((*p).extrns), da_slice((*p).funcs), da_slice((*p).asm_funcs), da_slice((*p).globals));
